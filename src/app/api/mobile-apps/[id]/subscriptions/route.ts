...
import { isGooglePlayConfigured, listSubscriptions, getSubscription } from "@/lib/google-play";

// GET: list subscriptions from Google Play
export async function GET(
  ...
  try {
    if (productId) {
      const sub = await getSubscription(packageName, productId);
      return NextResponse.json(sub);
    }
    const data = await listSubscriptions(packageName);
