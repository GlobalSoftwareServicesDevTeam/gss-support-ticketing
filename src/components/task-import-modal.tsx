...
interface ImportResult {
  ...
  errors: number;
  total: number;
  details?: { row: number; title: string; error: string }[];
}
...
export default function TaskImportModal({
  ...
  function downloadTemplate() {
    const csv = "Title,Description,Priority,Status,Start Date,Due Date,Assignee\nExample Task,Some details,MEDIUM,TODO,2026-04-20,2026-04-25,john@example.com\n";
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-gray-700">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold text-slate-900 dark:text-white">Import Tasks</h2>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          ...
          {step === "upload" && (
            <div className="space-y-4">
              <p className="text-sm text-slate-600 dark:text-gray-400">
                Import tasks from a CSV, Excel (.xlsx), or Google Sheets export file.
              </p>
          ...
          {step === "mapping" && preview && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <p className="text-sm text-slate-600 dark:text-gray-400">
                  Map your spreadsheet columns to task fields.
                </p>
                    ...
                    <div key={field.key} className="flex items-center gap-2">
                      ...
                      >
                        ...
                        {preview.headers.map((h) => (
                          <option key={h} value={h.trim().toLowerCase().replace(/[\s-]+/g, "")}>{h}</option>
                        ))}
                      </select>
                    </div>
                  ))}
          ...
          {step === "importing" && (
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <p className="text-sm text-slate-600 dark:text-gray-400">Importing tasks...</p>
            </div>
          )}
                ...
                <div className="bg-yellow-50 dark:bg-yellow-900/10 border border-yellow-200 dark:border-yellow-800 rounded-lg p-3">
                  ...
                  <ul className="space-y-1 text-xs text-yellow-700 dark:text-yellow-500">
                    {result.details.map((d, i) => (
                      <li key={i}>Row {d.row}: &quot;{d.title}&quot; — {d.error}</li>
                    ))}
                  </ul>
                </div>
              )}
          ...
          <div className="flex items-center gap-2">
            ...
            {step === "mapping" && (
              <button
                ...
                disabled={!projectId || !mapping.title || loading}
